...
class StudentListController(QDialog):
    ...
    def fill_table(self, students):
        ...
        for student in students:
            ...
            item = QTableWidgetItem(str(student.student_id))
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 1, item)
            item = QTableWidgetItem(student.full_name or "")
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 2, item)
            
            dob_str = student.date_of_birth.strftime("%d/%m/%Y") if hasattr(student.date_of_birth, "strftime") else str(student.date_of_birth or "")
            item = QTableWidgetItem(dob_str)
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 3, item)
            
            item = QTableWidgetItem(student.gender or "")
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 4, item)
            
            item = QTableWidgetItem(student.address or "")
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 5, item)
            
            item = QTableWidgetItem(student.phone_number or "")
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 6, item)
            
            item = QTableWidgetItem(student.email or "")
            item.setForeground(QBrush(QColor("#222")))
            table.setItem(row, 7, item)
            
            reg_date_str = student.register_date.strftime("%d/%m/%Y") if hasattr(student.register_date, "strftime") else str(student.register_date or "")
            item = QTableWidgetItem(reg_date_str)
            item.setForeground(QBrush(QColor("#222")))
